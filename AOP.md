...
- 当目标类有接口的情况时使用JDK动态代理和cglib动态代理，没有接口时只能使用cglib动态代理；
...
- AspectJ：是AOP思想的一种实现，本质上是静态代理，将代理逻辑植入被代理的目标类编译得到字节码文件，所以最终效果是动态的。wearer就是植入器，spring只是借用了AspectJ中的注解。

###### 切入点表达式

![](D:\JobHuntingJourney\projectlll\projectll\spring66\fig\fig4.jpg)